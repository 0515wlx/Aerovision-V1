...
AeroVision 是一个基于 YOLOv8x 的飞机识别系统，专注于目标检测和 OCR 识别。
...
- **基于 YOLOv8x**：使用 YOLOv8x 作为基座模型，提供最强大的检测能力
- **目标检测**：支持飞机检测、注册号检测等任务
- **OCR 识别**：集成 PaddleOCR 进行文字识别
...
│   └── utils/           # 工具函数
├── checkpoints/         # 模型检查点
│   └── registration_detection/    # 检测模型检查点
├── logs/               # 日志目录
├── tests/              # 测试代码
└── docs/               # 文档目录
...
### 模型训练
- **YOLOv8x 训练**：基于 YOLOv8x 进行目标检测训练
- **超参数调优**：支持自定义训练参数
