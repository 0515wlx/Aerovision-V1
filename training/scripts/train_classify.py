...
# Disable wandb early to avoid project name issues
os.environ["WANDB_MODE"] = "disabled"

import torch

# Fix PyTorch 2.6+ weights_only issue for loading ultralytics models
...
try:
    ...
    torch.serialization.add_safe_globals(
        [ClassificationModel, DetectionModel, SegmentationModel, PoseModel, OBBModel]
    )
except Exception:
    pass  # Ignore if classes don't exist or adding fails
...
class AircraftClassifierTrainer:
    ...
    def train(self) -> None:
        ...
        try:
            # Disable wandb to avoid project name issues
            os.environ["WANDB_MODE"] = "disabled"
...
def main() -> None:
    ...
    config = {
        ...
        or 3.0,
        # Device - 优先使用配置文件，确保在 CPU 版 PyTorch 下正确设置
        "device": config_obj.get("device.default") or args.device or "cpu",
